...
import com.moktar.mzpermission.R
import com.moktar.mzpermission.databinding.MzpermissionDefaultDialogLayoutBinding
import com.moktar.mzpermission.databinding.MzpermissionPermissionItemBinding
...
class DefaultDialog(
...
) : RationaleDialog(context, R.style.MzPermissionDefaultDialog) {

    private lateinit var binding: MzpermissionDefaultDialogLayoutBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = MzpermissionDefaultDialogLayoutBinding.inflate(layoutInflater)
     ...
     * But we only need to add the permission group. So if there're two permissions belong to one group,
     * only one item will be added to the dialog.
     */
    private fun buildPermissionsLayout() {
        ...
        for (permission in permissions) {
            ...
            if ((permission in allSpecialPermissions && !tempSet.contains(permission))
                ...
                val itemBinding = MzpermissionPermissionItemBinding
                    .inflate(layoutInflater, binding.permissionsLayout, false)
                when (permission) {
                    Manifest.permission.ACCESS_BACKGROUND_LOCATION -> {
                        itemBinding.permissionText.text =
                            context.getString(R.string.mzpermission_access_background_location)
                        itemBinding.permissionIcon
                            .setImageResource(R.drawable.mzpermission_ic_location)
                    }
                    Manifest.permission.SYSTEM_ALERT_WINDOW -> {
                        itemBinding.permissionText.text =
                            context.getString(R.string.mzpermission_system_alert_window)
                        itemBinding.permissionIcon.setImageResource(R.drawable.mzpermission_ic_alert)
                    }
                    Manifest.permission.WRITE_SETTINGS -> {
                        itemBinding.permissionText.text =
                            context.getString(R.string.mzpermission_write_settings)
                        itemBinding.permissionIcon
                            .setImageResource(R.drawable.mzpermission_ic_setting)
                    }
                    Manifest.permission.MANAGE_EXTERNAL_STORAGE -> {
                        itemBinding.permissionText.text =
                            context.getString(R.string.mzpermission_manage_external_storage)
                        itemBinding.permissionIcon.setImageResource(R.drawable.mzpermission_ic_storage)
                    }
                    Manifest.permission.REQUEST_INSTALL_PACKAGES -> {
                        itemBinding.permissionText.text =
                            context.getString(R.string.mzpermission_request_install_packages)
                        itemBinding.permissionIcon.setImageResource(R.drawable.mzpermission_ic_install)
                    }
                    else -> {
                        itemBinding.permissionText.text =
                            context.getString(
